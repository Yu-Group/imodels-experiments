...
def compare_estimators(estimators: List[ModelConfig],
    ...
    for model in estimators:
        ...
        for splitting_strategy, fi_ests in fi_ests_dict.items():
            ...
            if sklearn.base.is_classifier(est):
                rf_plus = RandomForestPlusClassifier(rf_model=est, prediction_model=LogisticRegressionCV(penalty='elasticnet', l1_ratios=[0.1,0.5,0.99], solver = 'saga', cv=3, n_jobs=-1, tol=5e-4, max_iter=2000, random_state=0))
                ...
                rf_plus = RandomForestPlusRegressor(rf_model=est, prediction_model=ElasticNetCV(cv=3, l1_ratio=[0.1,0.5,0.99], max_iter=2000, random_state=0))
                rf_plus.fit(X_train, y_train)
            ...
            for fi_est in tqdm(fi_ests):
                ...
                if fi_est.base_model == "RF":
                    loaded_model = est
                elif fi_est.base_model == "RFPlus":
                    loaded_model = rf_plus
                elif fi_est.base_model == "RFPlus_inbag":
                    loaded_model = rf_plus_inbag

                local_fi_score_train, local_fi_score_test = fi_est.cls(X_train=X_train, y_train=y_train, X_test=X_test, model=loaded_model, absolute=True)
...
if __name__ == '__main__':
    ...
    if default_dir is not None:
        default_dir = oj(default_dir, "feature_importance", "results_new")
    else:
        default_dir = oj(os.path.dirname(os.path.realpath(__file__)), 'results_new')
