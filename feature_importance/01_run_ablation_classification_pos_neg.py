...
def compare_estimators(estimators: List[ModelConfig],
    ...
    feature_importance_list_positive = {}
    feature_importance_list_negative = {}
        ...
        for splitting_strategy, fi_ests in fi_ests_dict.items():
            ...
            for fi_est in tqdm(fi_ests):
                ...
                if fi_est.name.startswith("Local_MDI+"):
                    ...
                    local_fi_score_train_subset_pos_neg = local_fi_score_train_pos_neg[indices_train]
                end = time.time()
                metric_results['fi_time'] = end - start
                print(f"done with feature importance: {end - start}")

                pos_train_subset_mask = local_fi_score_train_subset_pos_neg > 0
                neg_train_subset_mask = local_fi_score_train_subset_pos_neg < 0
                pos_test_subset_mask = local_fi_score_test_subset_pos_neg > 0
                neg_test_subset_mask = local_fi_score_test_subset_pos_neg < 0
                if isinstance(local_fi_score_test, np.ndarray):
                    pos_test_mask = local_fi_score_test_pos_neg > 0
                    neg_test_mask = local_fi_score_test_pos_neg < 0
            
                local_fi_score_train_subset_pos = local_fi_score_train_subset.copy()
                local_fi_score_train_subset_neg = local_fi_score_train_subset.copy()
                local_fi_score_test_subset_pos = local_fi_score_test_subset.copy()
                local_fi_score_test_subset_neg = local_fi_score_test_subset.copy()
                if isinstance(local_fi_score_test, np.ndarray):
                    local_fi_score_test_pos = local_fi_score_test.copy()
                    local_fi_score_test_neg = local_fi_score_test.copy()
                else:
                    local_fi_score_test_pos = local_fi_score_test
                    local_fi_score_test_neg = local_fi_score_test

                if fi_est.ascending:
                    local_fi_score_train_subset_pos[~pos_train_subset_mask] = 0
                    local_fi_score_train_subset_neg[~neg_train_subset_mask] = 0
                    local_fi_score_test_subset_pos[~pos_test_subset_mask] = 0
                    local_fi_score_test_subset_neg[~neg_test_subset_mask] = 0
                    if isinstance(local_fi_score_test, np.ndarray):
                        local_fi_score_test_pos[~pos_test_mask] = 0
                        local_fi_score_test_neg[~neg_test_mask] = 0
                else:
                    local_fi_score_train_subset_pos[~pos_train_subset_mask] = sys.maxsize-1
                    local_fi_score_train_subset_neg[~neg_train_subset_mask] = sys.maxsize-1
                    local_fi_score_test_subset_pos[~pos_test_subset_mask] = sys.maxsize-1
                    local_fi_score_test_subset_neg[~neg_test_subset_mask] = sys.maxsize-1
                    if isinstance(local_fi_score_test, np.ndarray):
                        local_fi_score_test_pos[~pos_test_mask] = sys.maxsize-1
                        local_fi_score_test_neg[~neg_test_mask] = sys.maxsize-1

                feature_importance_list_positive[fi_est.name] = [local_fi_score_train_subset_pos, local_fi_score_test_pos, local_fi_score_test_subset_pos]
                feature_importance_list_negative[fi_est.name] = [local_fi_score_train_subset_neg, local_fi_score_test_neg, local_fi_score_test_subset_neg]
                ...
                ablation_models = {"RF_Classifier": RandomForestClassifier(n_estimators=100, min_samples_leaf=1, max_features='sqrt', random_state=42),
                                   "LogisticCV": LogisticRegressionCV(random_state=42, max_iter=200),
                ...
                all_fi_pos = [local_fi_score_train_subset_pos, local_fi_score_test_subset_pos, local_fi_score_test_pos]
                all_fi_rank_pos = [None, None, None]
                for i in range(len(all_fi_pos)):
                    fi = all_fi_pos[i]
                    if isinstance(fi, np.ndarray):
                        fi[fi == float("-inf")] = -sys.maxsize - 1
                        fi[fi == float("inf")] = sys.maxsize - 1
                        if fi_est.ascending:
                            all_fi_rank_pos[i] = np.argsort(-fi)
                        else:
                            all_fi_rank_pos[i] = np.argsort(fi)

                feature_importance_list_positive[fi_est.name].extend(all_fi_rank_pos)


                all_fi_neg = [local_fi_score_train_subset_neg, local_fi_score_test_subset_neg, local_fi_score_test_neg]
                all_fi_rank_neg = [None, None, None]
                for i in range(len(all_fi_pos)):
                    fi = all_fi_neg[i]
                    if isinstance(fi, np.ndarray):
                        fi[fi == float("-inf")] = -sys.maxsize - 1
                        ...
                        if fi_est.ascending:
                            all_fi_rank_neg[i] = np.argsort(-fi)
                        else:
                            all_fi_rank_neg[i] = np.argsort(fi)

                feature_importance_list_negative[fi_est.name].extend(all_fi_rank_neg)
                            
                ablation_datas = {"train_subset": (X_train_subset, y_train_subset, all_fi_rank_pos[0], all_fi_pos[0]),
                                 "test_subset": (X_test_subset, y_test_subset, all_fi_rank_pos[1], all_fi_pos[1]),
                                 "test": (X_test, y_test, all_fi_rank_pos[2], all_fi_pos[2])}
                ...
                for ablation_data in ablation_datas:
                    start = time.time()
                    X_data, y_data, local_fi_score_data, local_fi_raw = ablation_datas[ablation_data]
                    if not isinstance(local_fi_score_data, np.ndarray):
                        for a_model in ablation_models:
                            metric_results[a_model + f'_{ablation_data}_AUROC_before_ablation_positive'] = None
                            metric_results[a_model + f'_{ablation_data}_AUPRC_before_ablation_positive'] = None
                            metric_results[a_model + f'_{ablation_data}_F1_before_ablation_positive'] = None
                            ###
                            metric_results[a_model + f'_{ablation_data}_mean_y_pred_before_ablation_positive'] = None
                            ###
                        for i in range(num_ablate_features):
                            for a_model in ablation_models:
                                metric_results[f'{a_model}_{ablation_data}_AUROC_after_ablation_{i+1}_positive'] = None
                                metric_results[f'{a_model}_{ablation_data}_AUPRC_after_ablation_{i+1}_positive'] = None
                                metric_results[f'{a_model}_{ablation_data}_F1_after_ablation_{i+1}_positive'] = None
                                ###
                                metric_results[f'{a_model}_{ablation_data}_mean_y_pred_after_ablation_{i+1}_positive'] = None
                                ###
                    else:
                        for a_model in ablation_models:
                            ...
                            metric_results[a_model + f'_{ablation_data}_AUROC_before_ablation_positive'] = roc_auc_score(y_data, y_pred)
                            metric_results[a_model + f'_{ablation_data}_AUPRC_before_ablation_positive'] = average_precision_score(y_data, y_pred)
                            metric_results[a_model + f'_{ablation_data}_F1_before_ablation_positive'] = f1_score(y_data, y_pred > 0.5)
                            ###
                            metric_results[a_model + f'_{ablation_data}_mean_y_pred_before_ablation_positive'] = np.mean(y_pred)
                            ###
                            imp_vals = copy.deepcopy(local_fi_score_data)
                            local_fi_raw_copy = copy.deepcopy(local_fi_raw)
                            ...
                            ###
                            ablation_results_list_mean_y_pred = [0] * num_ablate_features
                            ###
                            X_temp = X_data.copy()
                            for i in range(num_ablate_features):
                                ablation_X_data = ablation_removal_pos_neg(train_mean, X_temp, imp_vals, local_fi_raw_copy, i)
                                ...
                                ###
                                ablation_results_list_mean_y_pred[i] = np.mean(ablation_est.predict(ablation_X_data))
                                ###
                                X_temp = ablation_X_data
                            for i in range(num_ablate_features):
                                metric_results[f'{a_model}_{ablation_data}_AUROC_after_ablation_{i+1}_positive'] = ablation_results_auroc_list[i]
                                metric_results[f'{a_model}_{ablation_data}_AUPRC_after_ablation_{i+1}_positive'] = ablation_results_auprc_list[i]
                                metric_results[f'{a_model}_{ablation_data}_F1_after_ablation_{i+1}_positive'] = ablation_results_f1_list[i]
                                ###
                                metric_results[f'{a_model}_{ablation_data}_mean_y_pred_after_ablation_{i+1}_positive'] = ablation_results_list_mean_y_pred[i]
                                ###
                    end = time.time()
                    print(f"done with ablation removal: {ablation_data} {end - start}")
                    metric_results[f'{ablation_data}_ablation_removal_time_positive'] = end - start


                ablation_datas = {"train_subset": (X_train_subset, y_train_subset, all_fi_rank_neg[0], all_fi_neg[0]),
                                  "test_subset": (X_test_subset, y_test_subset, all_fi_rank_neg[1], all_fi_neg[1]),
                                  "test": (X_test, y_test, all_fi_rank_neg[2], all_fi_neg[2])}
                ...
                for ablation_data in ablation_datas:
                    ...
                    if not isinstance(local_fi_score_data, np.ndarray):
                        for a_model in ablation_models:
                            metric_results[a_model + f'_{ablation_data}_AUROC_before_ablation_negative'] = None
                            metric_results[a_model + f'_{ablation_data}_AUPRC_before_ablation_negative'] = None
                            metric_results[a_model + f'_{ablation_data}_F1_before_ablation_negative'] = None
                            ###
                            metric_results[a_model + f'_{ablation_data}_mean_y_pred_before_ablation_negative'] = None
                            ###
                        for i in range(num_ablate_features):
                            for a_model in ablation_models:
                                metric_results[f'{a_model}_{ablation_data}_AUROC_after_ablation_{i+1}_negative'] = None
                                metric_results[f'{a_model}_{ablation_data}_AUPRC_after_ablation_{i+1}_negative'] = None
                                metric_results[f'{a_model}_{ablation_data}_F1_after_ablation_{i+1}_negative'] = None
                                ###
                                metric_results[f'{a_model}_{ablation_data}_mean_y_pred_after_ablation_{i+1}_negative'] = None
                                ###
                    else:
                        for a_model in ablation_models:
                            print(f"start ablation removal: {ablation_data} {a_model}")
                            ablation_est = ablation_models[a_model]
                            y_pred = ablation_est.predict(X_data)
                            metric_results[a_model + f'_{ablation_data}_AUROC_before_ablation_negative'] = roc_auc_score(y_data, y_pred)
                            metric_results[a_model + f'_{ablation_data}_AUPRC_before_ablation_negative'] = average_precision_score(y_data, y_pred)
                            metric_results[a_model + f'_{ablation_data}_F1_before_ablation_negative'] = f1_score(y_data, y_pred > 0.5)
                            ###
                            metric_results[a_model + f'_{ablation_data}_mean_y_pred_before_ablation_negative'] = np.mean(y_pred)
                            ###
                            imp_vals = copy.deepcopy(local_fi_score_data)
                            local_fi_raw_copy = copy.deepcopy(local_fi_raw)
                            ablation_results_auroc_list = [0] * num_ablate_features
                            ...
                            for i in range(num_ablate_features):
                                ...
                                X_temp = ablation_X_data
                            for i in range(num_ablate_features):
                                metric_results[f'{a_model}_{ablation_data}_AUROC_after_ablation_{i+1}_negative'] = ablation_results_auroc_list[i]
                                metric_results[f'{a_model}_{ablation_data}_AUPRC_after_ablation_{i+1}_negative'] = ablation_results_auprc_list[i]
                                metric_results[f'{a_model}_{ablation_data}_F1_after_ablation_{i+1}_negative'] = ablation_results_f1_list[i]
                                ###
                                metric_results[f'{a_model}_{ablation_data}_mean_y_pred_after_ablation_{i+1}_negative'] = ablation_results_list_mean_y_pred[i]
                                ###
                    end = time.time()
                    print(f"done with ablation removal: {ablation_data} {end - start}")
                    metric_results[f'{ablation_data}_ablation_removal_time_negative'] = end - start
                ...
                #     if not isinstance(local_fi_score_data, np.ndarray):
                #         for a_model in ablation_models:
                #             metric_results[a_model + f'_{ablation_data}_AUROC_before_ablation_addition'] = None
                #             metric_results[a_model + f'_{ablation_data}_AUPRC_before_ablation_addition'] = None
                #         for i in range(num_ablate_features):
                #             for a_model in ablation_models:
                ...
                #             metric_results[a_model + f'_{ablation_data}_AUROC_before_ablation_addition'] = mean_squared_error(y_data, y_pred)
                #             metric_results[a_model + f'_{ablation_data}_AUPRC_before_ablation_addition'] = r2_score(y_data, y_pred)
                #             imp_vals = copy.deepcopy(local_fi_score_data)
                ...
                #                 X_temp = ablation_X_data
                #             for i in range(num_ablate_features):
                #                 metric_results[f'{a_model}_{ablation_data}_AUROC_after_ablation_{i+1}_addition'] = ablation_results_list[i]
                #                 metric_results[f'{a_model}_{ablation_data}_AUPRC_after_ablation_{i+1}_addition'] = ablation_results_list_r2[i]
                #     end = time.time()
    ...
    return results, feature_importance_list_positive, feature_importance_list_negative
...
def run_comparison(path: str,
    ...
    results, fi_lst_pos, fi_lst_neg = compare_estimators(estimators=estimators,
    ...
    for i in range(len(feature_importance_all)):
        pkl.dump(list(fi_lst_pos.items())[i], open(feature_importance_all[i], 'wb'))
        pkl.dump(list(fi_lst_neg.items())[i], open(feature_importance_all[i], 'wb'))
