...
def ablation_removal_pos(train_mean, data, feature_importance_rank_pos, feature_importance, feature_index):
    data_copy = data.copy()
    indices = feature_importance_rank_pos[:, feature_index]
    sum = 0
    for i in range(data.shape[0]):
        if feature_importance[i, indices[i]] > 0:
            sum += 1
            data_copy[i, indices[i]] = train_mean[indices[i]]
    print("Remove sum pos: ", sum)
    return data_copy

def ablation_removal_neg(train_mean, data, feature_importance_rank_neg, feature_importance, feature_index):
    data_copy = data.copy()
    indices = feature_importance_rank_neg[:, feature_index]
    sum = 0
    for i in range(data.shape[0]):
        if feature_importance[i, indices[i]] < 0:
            sum += 1
            data_copy[i, indices[i]] = train_mean[indices[i]]
    print("Remove sum neg: ", sum)
    return data_copy
        ...
        for splitting_strategy, fi_ests in fi_ests_dict.items():
            ...
            if args.absolute_masking or args.positive_masking or args.negative_masking:
                ...
                for fi_est in tqdm(fi_ests):
                    ...
                    #ablation_datas = {"train_subset": (X_train_subset, y_train_subset, all_fi[0], all_fi_rank_pos[0], all_fi_rank_neg[0]),
                    ablation_datas = {"test_subset": (X_test_subset, y_test_subset, all_fi[1], all_fi_rank_pos[1], all_fi_rank_neg[1]),
                                    "test": (X_test, y_test, all_fi[2], all_fi_rank_pos[2], all_fi_rank_neg[2])}
                    ...
                    for ablation_data in ablation_datas:
                        start = time.time()
                        X_data, y_data, local_fi_score, local_fi_score_rank_pos, local_fi_score_rank_neg = ablation_datas[ablation_data]
                        if not isinstance(local_fi_score, np.ndarray):
                            for a_model in ablation_models:
                                for i in range(num_features_masked+1):
                                    metric_results[f'{a_model}_{ablation_data}_correct_prediction_log_loss_after_ablation_{i}'] = None
                                    metric_results[f'{a_model}_{ablation_data}_incorrect_prediction_log_loss_after_ablation_{i}'] = None
                        else:
                            for a_model in ablation_models:
                                ...
                                # Find indices of samples that are correctly and incorrectly predicted
                                # print accuracy
                                print(f"Accuracy before ablation: {np.mean(y_data == (y_pred_before > 0.5))}")
                                correct_indices = np.where(y_data == (y_pred_before > 0.5))[0]
                                incorrect_indices = np.where(y_data != (y_pred_before > 0.5))[0]
                                y_0_indices = np.where(y_data == 0)[0]
                                y_1_indices = np.where(y_data == 1)[0]
                                X_data_correct_y_0 = X_data[np.intersect1d(correct_indices, y_0_indices)]
                                print(X_data_correct_y_0.shape)
                                X_data_correct_y_1 = X_data[np.intersect1d(correct_indices, y_1_indices)]
                                print(X_data_correct_y_1.shape)
                                X_data_incorrect_y_0 = X_data[np.intersect1d(incorrect_indices, y_0_indices)]
                                print(X_data_incorrect_y_0.shape)
                                X_data_incorrect_y_1 = X_data[np.intersect1d(incorrect_indices, y_1_indices)]
                                print(X_data_incorrect_y_1.shape)

                                y_data_correct_y_0 = y_data[np.intersect1d(correct_indices, y_0_indices)]
                                y_data_correct_y_1 = y_data[np.intersect1d(correct_indices, y_1_indices)]
                                y_data_incorrect_y_0 = y_data[np.intersect1d(incorrect_indices, y_0_indices)]
                                y_data_incorrect_y_1 = y_data[np.intersect1d(incorrect_indices, y_1_indices)]

                                local_fi_score_correct_y_0 = local_fi_score[np.intersect1d(correct_indices, y_0_indices)]
                                local_fi_score_correct_y_1 = local_fi_score[np.intersect1d(correct_indices, y_1_indices)]
                                local_fi_score_incorrect_y_0 = local_fi_score[np.intersect1d(incorrect_indices, y_0_indices)]
                                local_fi_score_incorrect_y_1 = local_fi_score[np.intersect1d(incorrect_indices, y_1_indices)]

                                local_fi_score_correct_y_0_rank_neg = local_fi_score_rank_neg[np.intersect1d(correct_indices, y_0_indices)]
                                local_fi_score_correct_y_1_rank_pos = local_fi_score_rank_pos[np.intersect1d(correct_indices, y_1_indices)]
                                local_fi_score_incorrect_y_0_rank_pos = local_fi_score_rank_pos[np.intersect1d(incorrect_indices, y_0_indices)]
                                local_fi_score_incorrect_y_1_rank_neg = local_fi_score_rank_neg[np.intersect1d(incorrect_indices, y_1_indices)]


                                X_data_correct_combined = np.concatenate([X_data_correct_y_0, X_data_correct_y_1])
                                y_data_correct_combined = np.concatenate([y_data_correct_y_0, y_data_correct_y_1])
                                X_data_incorrect_combined = np.concatenate([X_data_incorrect_y_0, X_data_incorrect_y_1])
                                y_data_incorrect_combined = np.concatenate([y_data_incorrect_y_0, y_data_incorrect_y_1])

                                metric_results[f'{a_model}_{ablation_data}_correct_prediction_log_loss_after_ablation_0'] = log_loss(y_data_correct_combined, ablation_est.predict_proba(X_data_correct_combined)[:, 1])
                                metric_results[f'{a_model}_{ablation_data}_incorrect_prediction_log_loss_after_ablation_0'] = log_loss(y_data_incorrect_combined, ablation_est.predict_proba(X_data_incorrect_combined)[:, 1])
                                X_data_correct_y_0_temp = copy.deepcopy(X_data_correct_y_0)
                                X_data_correct_y_1_temp = copy.deepcopy(X_data_correct_y_1)
                                X_data_incorrect_y_0_temp = copy.deepcopy(X_data_incorrect_y_0)
                                X_data_incorrect_y_1_temp = copy.deepcopy(X_data_incorrect_y_1)
                                for i in range(num_features_masked):
                                    ablation_X_data_correct_y_0 = ablation_removal_neg(train_mean, X_data_correct_y_0_temp, local_fi_score_correct_y_0_rank_neg, local_fi_score_correct_y_0, i)
                                    ablation_X_data_correct_y_1 = ablation_removal_pos(train_mean, X_data_correct_y_1_temp, local_fi_score_correct_y_1_rank_pos, local_fi_score_correct_y_1, i)
                                    ablation_X_data_incorrect_y_0 = ablation_removal_pos(train_mean, X_data_incorrect_y_0_temp, local_fi_score_incorrect_y_0_rank_pos, local_fi_score_incorrect_y_0, i)
                                    ablation_X_data_incorrect_y_1 = ablation_removal_neg(train_mean, X_data_incorrect_y_1_temp, local_fi_score_incorrect_y_1_rank_neg, local_fi_score_incorrect_y_1, i)
                                    X_data_correct_combined = np.concatenate([ablation_X_data_correct_y_0, ablation_X_data_correct_y_1])
                                    X_data_incorrect_combined = np.concatenate([ablation_X_data_incorrect_y_0, ablation_X_data_incorrect_y_1])
                                    metric_results[f'{a_model}_{ablation_data}_correct_prediction_log_loss_after_ablation_{i+1}'] = log_loss(y_data_correct_combined, ablation_est.predict_proba(X_data_correct_combined)[:, 1])
                                    metric_results[f'{a_model}_{ablation_data}_incorrect_prediction_log_loss_after_ablation_{i+1}'] = log_loss(y_data_incorrect_combined, ablation_est.predict_proba(X_data_incorrect_combined)[:, 1])
                                    X_data_correct_y_0_temp = copy.deepcopy(ablation_X_data_correct_y_0)
                                    X_data_correct_y_1_temp = copy.deepcopy(ablation_X_data_correct_y_1)
                                    X_data_incorrect_y_0_temp = copy.deepcopy(ablation_X_data_incorrect_y_0)
                                    X_data_incorrect_y_1_temp = copy.deepcopy(ablation_X_data_incorrect_y_1)
                        end = time.time()
