        ...
        for splitting_strategy, fi_ests in fi_ests_dict.items():
            ...
            if args.fit_model:
                ...
                fitted_results = {
                        ...
                        "X_seed": [args.x_seed, args.x_seed, args.x_seed],
                        "Y_seed": [args.y_seed, args.y_seed, args.y_seed],
                        "Split_seed": [args.split_seed, args.split_seed, args.split_seed]
                }
                temp = ""
                ...
                results_df.to_csv(f"/scratch/users/zhongyuan_liang/saved_models/auroc/{args.folder_name}/RFPlus_fitted_summary_{args.x_seed}_{args.y_seed}_{args.split_seed}_{temp}.csv", index=False)
            ...
            for fi_est in tqdm(fi_ests):
                ...
                for d in all_fi_data:
                    ...
                    ground_truth_fi = ground_truth_fi_derivation(x_data, support, args.dgp_fi)
                    if not isinstance(fi_data, np.ndarray):
                        ...
                        for k in range(num_signal_features):
                            metric_results[f'partial_auroc_{d}_{k}'] = None
                    else:
                        auroc = []
                        rbo_lst_09 = []
                        parital_auroc = [0]*num_signal_features
                        for i in range(fi_data.shape[0]):
                            ...
                            if fi_est.ascending:
                                ...
                                for k in range(num_signal_features):
                                    partial_auroc_ground_truth = encode_largest_k(ground_truth_fi_i, k+1)
                                    parital_auroc[k] += roc_auc_score(partial_auroc_ground_truth, fi_data_i)
                            else:
                                ...
                                for k in range(num_signal_features):
                                    partial_auroc_ground_truth = encode_largest_k(ground_truth_fi_i, k+1)
                                    parital_auroc[k] += roc_auc_score(partial_auroc_ground_truth, -1*fi_data_i)
                        metric_results[f'auroc_{d}'] = np.array(auroc).mean()
                        metric_results[f'rbo_09_{d}'] = np.array(rbo_lst_09).mean()
                        for k in range(num_signal_features):
                            parital_auroc[k] /= fi_data.shape[0]
                            metric_results[f'partial_auroc_{d}_{k}'] = parital_auroc[k]
                    print(f"{fi_est.name} done with {d} data with auroc: {metric_results[f'auroc_{d}']}, and rbo: {metric_results[f'rbo_09_{d}']}")
...
def run_simulation(i, path, val_name, X_params_dict, X_dgp, y_params_dict, y_dgp, ests, fi_ests, metrics, args, vary_setting):
    ...
    while iter <= max_iter:  # regenerate data if y is constant
        X = X_dgp(**X_params_dict, seed = args.x_seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--x_seed', type=int, default=0)
    parser.add_argument('--y_seed', type=int, default=0)

    parser.add_argument('--dgp_fi', type=str, default=None)
    ...
    if isinstance(vary_param_name, list):
        ...
        path = oj(results_dir, "varying_" + "_".join(vary_param_name), "seed" + str(args.x_seed)+ str(args.y_seed)+ str(args.split_seed))
    else:
        #path = oj(results_dir, "varying_" + vary_param_name, "seed" + str(args.split_seed))
        path = oj(results_dir, "varying_" + vary_param_name, "seed" + str(args.x_seed)+ str(args.y_seed)+ str(args.split_seed))
