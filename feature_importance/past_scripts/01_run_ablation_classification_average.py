                    ...
                    for m in mode:
                        ...
                        for mask in mask_ratio:
                            ...
                            for a_model in ablation_models:
                                ...
                                y_pred_proba = ablation_models[a_model].predict_proba(X_test_masked)[:, 1]
                                metric_results[f'{a_model}_AUROC_{mask}_{m}'] = roc_auc_score(y_test, y_pred_proba)
                                metric_results[f'{a_model}_AUPRC_{mask}_{m}'] = average_precision_score(y_test, y_pred_proba)
                                metric_results[f'{a_model}_F1_{mask}_{m}'] = f1_score(y_test, y_pred_proba > 0.5)
                                metric_results[f'{a_model}_logloss_{mask}_{m}'] = log_loss(y_test, y_pred_proba)
