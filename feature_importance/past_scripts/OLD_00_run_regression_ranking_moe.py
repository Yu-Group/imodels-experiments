...
### MOE imports
from imodels.tree.rf_plus.rf_plus.MOE.rfplus_MOE import SklearnRFPlusRegMOEGrid
import shutil
...
def compare_estimators(estimators: List[ModelConfig],
    ...
    for model in estimators:
        ...
        for splitting_strategy, fi_ests in fi_ests_dict.items():
            ...
            rf_plus_moe_elasticnet = SklearnRFPlusRegMOEGrid(param_grid={'rfplus_model':[rf_plus_elastic],'lr':[1e-2, 1e-3, 1e-4], 'checkpoint_path': [os.path.join(args.checkpoint_path, "rfplus_elastic")]})
            rf_plus_moe_elasticnet.fit(X_train,y_train)

            rf_plus_moe_default = SklearnRFPlusRegMOEGrid(param_grid={'rfplus_model':[rf_plus_default],'lr':[1e-2, 1e-3, 1e-4], 'checkpoint_path': [os.path.join(args.checkpoint_path, "rfplus_default")]})
            rf_plus_moe_default.fit(X_train,y_train)

            rf_plus_moe_loo_default = SklearnRFPlusRegMOEGrid(param_grid={'rfplus_model':[rf_plus_default],'lr':[1e-2, 1e-3, 1e-4], 'checkpoint_path': [os.path.join(args.checkpoint_path, "rfplus_default_loo")], 'use_loo':[True]})
            rf_plus_moe_loo_default.fit(X_train,y_train)

            # rf_plus_moe_elasticnet = SklearnRFPlusRegMOEGrid(rfplus_model=rf_plus_elastic, checkpoint_path=os.path.join(args.checkpoint_path, "rfplus_elastic"))
            # rf_plus_moe_elasticnet.fit(X_train,y_train)

            # rf_plus_moe_default = SklearnRFPlusRegMOEGrid(rfplus_model=rf_plus_default, checkpoint_path=os.path.join(args.checkpoint_path, "rfplus_default"))
            # rf_plus_moe_default.fit(X_train,y_train)

            # rf_plus_moe_loo_default = SklearnRFPlusRegMOEGrid(rfplus_model=rf_plus_default, checkpoint_path=os.path.join(args.checkpoint_path, "rfplus_default_loo"), use_loo=True)
            # rf_plus_moe_loo_default.fit(X_train,y_train)
            ...
            rf_plus_moe_loo_default_r2 = r2_score(y_test, rf_plus_moe_loo_default.predict(X_test))
            ...
            for fi_est in tqdm(fi_ests):
                metric_results = {
                    ...
                    'est_r2': est_r2,
                    'rf_plus_elastic_r2': rf_plus_elastic_r2,
                    'rf_plus_default_r2': rf_plus_default_r2,
                    'rf_plus_inbag_r2': rf_plus_inbag_r2,
                    'rf_plus_moe_elasticnet_r2': rf_plus_moe_elasticnet_r2,
                    'rf_plus_moe_default_r2': rf_plus_moe_default_r2,
                    'rf_plus_moe_loo_default_r2': rf_plus_moe_loo_default_r2,
                }
                ...
                if fi_est.base_model == "None":
                    ...
                    loaded_model = rf_plus_default
                elif fi_est.base_model == "RFPlus_moe_elasticnet":
                    loaded_model = rf_plus_moe_elasticnet
                elif fi_est.base_model == "RFPlus_moe_default":
                    loaded_model = rf_plus_moe_default
                elif fi_est.base_model == "RFPlus_moe_loo_default":
                    loaded_model = rf_plus_moe_loo_default
            ...
            checkpoint_dir = os.path.join(args.checkpoint_path, "rfplus_elastic")
            shutil.rmtree(checkpoint_dir)  
            checkpoint_dir = os.path.join(args.checkpoint_path, "rfplus_default")
            shutil.rmtree(checkpoint_dir)  
            checkpoint_dir = os.path.join(args.checkpoint_path, "rfplus_default_loo")
            shutil.rmtree(checkpoint_dir)
...
def run_simulation(i, path, val_name, X_params_dict, X_dgp, y_params_dict, y_dgp, ests, fi_ests, metrics, args, vary_setting):
    ...
    while iter <= max_iter:  # regenerate data if y is constant
        X = X_dgp(**X_params_dict, seed = args.sample_seed)
...
if __name__ == '__main__':
    ...
    args.results_path = "/scratch/users/zhongyuan_liang/results_03_30"
    ...
    if isinstance(vary_param_name, list):
        ...
        path = oj(results_dir, "varying_" + "_".join(vary_param_name), "seed_" + str(args.feature_seed)+ "_" + str(args.sample_seed))
    else:
        #path = oj(results_dir, "varying_" + vary_param_name, "seed" + str(args.split_seed))
        path = oj(results_dir, "varying_" + vary_param_name, "seed_" + str(args.feature_seed)+ "_" + str(args.sample_seed))   
