    ...
    for model in estimators:
        ...
        for splitting_strategy, fi_ests in fi_ests_dict.items():
            ...
            for fi_est in tqdm(fi_ests):
                ...
                #     loaded_model = rf_plus_ridge
                # elif fi_est.base_model == "RFPlus_lasso":
                #     loaded_model = rf_plus_lasso
                ...
                _, local_fi_score_test = fi_est.cls(X_train=X_train, y_train=y_train, X_test=X_test, fit=loaded_model, mode="absolute")
                ...
                ablation_models = {"RF_Regressor_Default": RandomForestRegressor(random_state=args.rf_seed),
                                    "RF_Regressor": RandomForestRegressor(n_estimators=100, min_samples_leaf=5, max_features=0.33, random_state=args.rf_seed)}
                ...
                for mask in mask_ratio:
                    num_features_masked, X_test_masked = mask_unimportant_features(X_test, sorted_feature_test, mask, mask_values)
...
if __name__ == '__main__':
    ...
    if isinstance(vary_param_name, list):
        path = oj(results_dir, "varying_" + "_".join(vary_param_name), "seed_" + str(args.split_seed)+"_" + str(args.rf_seed)+ "_" + str(args.sample_seed))
    else:
        path = oj(results_dir, "varying_" + vary_param_name, "seed_" + str(args.split_seed)+"_" + str(args.rf_seed)+ "_" + str(args.sample_seed))
