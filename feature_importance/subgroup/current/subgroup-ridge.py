        ...
        for variant, nclust_map in train_clusters.items():
            ...
            for nclust in range(2, 11):
                ...
                for c in range(nclust):
                    ...
                    if task == "regression" and metric_name == "rmse" and variant == "shap" and nclust == 3:
                        # print cluster size
            ...
            for c, idxs in cluster_map.items():
                if len(idxs) < 3:
                    ...
                    warnings.warn(f"For {nclust} clusters, cluster #{c} in " + \
                        ...
                        Warning)
                # shuffle the indices and split them in train/test
                np.random.seed(1)
                np.random.shuffle(idxs)
                # make cutoff so that 1/4 of data is test and 3/4 is train
                cutoff = len(idxs) // 4
                fitting_c_to_idxs[c] = idxs[cutoff:]
                evaluation_c_to_idxs[c] = idxs[:cutoff]
                print("train size:", len(fitting_c_to_idxs[c]))
                print("test size:", len(evaluation_c_to_idxs[c]))
                # cutoff = len(idxs) // 2
                # fitting_c_to_idxs[c] = idxs[cutoff:]
                # evaluation_c_to_idxs[c] = idxs[:cutoff]
