...
if (!dir.exists(file.path(figures_dir, "appendix"))) {
  dir.create(file.path(figures_dir, "appendix"), recursive = TRUE)
}
...
for (y_model in y_models) {
  ...
  for (x_model in x_models) {
    ...
    if (params$for_paper) {
      ...
      ggplot2::ggsave(
        file.path(figures_dir, sprintf("regression_sims_%s_%s_%s_errbars.pdf",
...
for (y_model in y_models) {
  ...
  for (x_model in x_models) {
    ...
    if (params$for_paper) {
      ...
      plt <- patchwork::wrap_plots(plt_ls, nrow = 1)
      ggplot2::ggsave(
        file.path(figures_dir, sprintf("classification_sims_%s_%s_%s_errbars.pdf",
                                     y_model, x_model, metric)),
        plot = plt, units = "in", width = 14, height = height
      )
    } else {
  ...
  for (x_model in x_models) {
    ...
    for (mean_shift in mean_shifts) {
      ...
      if (params$for_paper) {
        ...
        plt <- patchwork::wrap_plots(plt_ls, nrow = 1)
        ggplot2::ggsave(
          file.path(figures_dir, sprintf("robust_sims_%s_%s_%sMS_%s_errbars.pdf",
                                       y_model, x_model, mean_shift, metric)),
          plot = plt, units = "in", width = 14, height = height
        )
      } else {
        plt <- results %>%
...
for (idx in 1:length(keep_heritabilities)) {
  ...
  if (params$for_paper) {
    ggplot2::ggsave(plt_wide, 
                    filename = file.path(figures_dir, "appendix", sprintf("correlation_sim_pve%s_wide_appendix.pdf", h)),
                    # height = fig_height * .55, width = fig_width * .25 * length(show_methods))
...
plt_base <- plot_perturbation_stability(
  ...
  facet_cols = "n_name", 
  group_fun = custom_group_fun, 
  descending_methods = c("GMDI (logistic, LOO)", "GMDI (logistic, in-bag)"),
  param_name = NULL,
  plot_types = "errbar",
  manual_color_palette = manual_color_palette,
  show_methods = show_methods,
...
plt <- plt_regression + plt_classification
if (params$for_paper) {
  ggplot2::ggsave(plt, filename = file.path(figures_dir, "appendix",
                                            "entropy_sims_appendix.pdf"), 
...
if (params$for_paper) {
  ...
  for (id in names(keep_methods_list)) {
    ...
    if (id == "no_mdioob") {
      ...
      ggplot2::ggsave(plt, 
                      filename = file.path(figures_dir, "appendix", sprintf("ccle_rnaseq_stability_select_features_%s.pdf", id)),
...
for (y_model in y_models) {
  ...
  for (x_model in x_models) {
    ...
    if (params$for_paper) {
      ...
      plt <- patchwork::wrap_plots(plt_ls, nrow = 1)
      ggplot2::ggsave(
        file.path(figures_dir, "appendix", 
...
# GMDI Modeling Choices {.tabset .tabset-vmodern}
