...
show_methods <- c("gjMDI_ridge_norm_loocv_RF", "Permutation_RF", "MDI_RF", "MDI-oob_RF", "TreeSHAP_RF")
...
for(i in y_dgp){
  sim_name <- sprintf("normal_block_cor_%s_dgp", y_dgp)
  fname <- file.path(results_dir, paste0("r2f.stability_sims.data_resampling_experiment.",
                                         sim_name), 
  ...
  rankings <- results %>%
    dplyr::group_by(heritability, rho_name, fi, rep) %>%
  ...
  rankings <- rankings %>%
    dplyr::group_by(group, rep, heritability, rho_name, fi) %>%
  ...
  ranks.sub$rho_name = factor(ranks.sub$rho_name,levels = c(0.5,0.8,0.99))
