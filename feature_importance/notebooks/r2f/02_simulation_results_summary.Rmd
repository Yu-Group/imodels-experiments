...
view_results <- function(metrics_plt_ls, 
                         ...
                         tpr_plt_ls = list(),
                         datasets = c("German Credit" = "german",
                                      "UK Biobank" = "ukbb",
                                      "CCLE" = "ccle_prot",
                                      "fMRI" = "fmri")) {
...
for (results_subdir in list.dirs(results_dir, full.names = TRUE, recursive = FALSE)) {
  if (stringr::str_detect(basename(results_subdir), "normal|omitted|algorithmic_choices|sparsity_sims|stability_sims")) {
    next
  }
...
# Artificial Simulations {.tabset .tabset-vmodern}
...
rmetrics_plt_ls <- list()
for (results_subdir in list.dirs(results_dir, full.names = TRUE, recursive = FALSE)) {
  if (!stringr::str_detect(basename(results_subdir), "normal|boolean")) {
    next
  }
  for (vary_param_dir in list.dirs(results_subdir, full.names = FALSE, recursive = FALSE)) {
    # print(file.path(results_subdir, vary_param_dir))
    ...
    facet_str <- "heritability"
    x_str <- "n"
    if (stringr::str_detect(sim_name, "rho")) {
      x_str <- "rho"
    }
...
view_results(metrics_plt_ls, rmetrics_plt_ls, datasets = c("Normal" = "normal"))
