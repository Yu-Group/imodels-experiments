      ...
      fi_scores = mapply(name = fi, scores_df = fi_scores,
                         function(name, scores_df) {
                           if (stringr::str_detect(name, "R2F")) {
                             scores_df <- scores_df %>% 
                               dplyr::mutate(ranking = rank(-r2, ties.method = "random"),
                                             importance = r2)
                           } else if (name %in% c("T-Test")) {
                             scores_df <- scores_df %>% 
                               dplyr::mutate(ranking = rank(importance, ties.method = "random"))
                           } else {
                             scores_df <- scores_df %>%
                               dplyr::mutate(ranking = rank(-importance, ties.method = "random"))
                           }
                           scores_df <- scores_df %>%
                             ...
                             # dplyr::mutate(.tp = cumsum(true_support))
                             dplyr::mutate(.tp = cumsum(true_support) / sum(true_support))
                           return(scores_df)
                         }, SIMPLIFY = FALSE)
    ) %>%
...
plot_ecdf <- function(results, vary_param_name, vary_param_name_vec, 
                      keep_vars = NULL) {
  if (!is.null(vary_param_name_vec)) {
    vary_param_name <- vary_param_name_vec
  }
  ...
  if (is.null(vary_param_name_vec)) {
    ...
    for (val1 in unique(plt_df[[paste0(vary_param_name[1], "_name")]])) {
      ...
      for (val2 in unique(plt_df[[paste0(vary_param_name[2], "_name")]])) {
        plt_ls[[as.character(val1)]][[as.character(val2)]] <- plt_df %>%
          dplyr::filter(
...
plot_feature_importance <- function(results, vary_param_name, vary_param_name_vec,
                                    keep_vars = NULL, 
...
view_results <- function(results_ls, metrics_plt_ls, metrics_errbar_plt_ls,
                         tpr_plt_ls, power_plt_ls, fi_bar_plt_ls, fi_box_plt_ls,
  ...
  for (sim_name in names(results_ls)) {
    ...
    if (is.null(vary_param_name_vec)) {
      ...
      for (param_val1 in names(power_plt_ls[[sim_name]])) {
        cat(sprintf("\n\n#### %s = %s {.tabset .tabset-pills .tabset-circle}\n\n\n",
                    vary_param_name_vec[1], param_val1))
        for (param_val2 in names(power_plt_ls[[sim_name]][[param_val1]])) {
          cat(sprintf("\n\n##### %s = %s\n\n\n", 
          ...
          if (interactive) {
            ...
            if (!abridged) {
              ...
              vthemes::subchunkify(plotly::ggplotly(fi_bar_plt_ls[[sim_name]][[param_val1]][[param_val2]]),
                                   ...
                                   add_class = "panel panel-default padded-panel")
              chunk_idx <<- chunk_idx + 1
              vthemes::subchunkify(plotly::ggplotly(power_plt_ls[[sim_name]][[param_val1]][[param_val2]]),
                                   i = chunk_idx, other_args = "out.width = '100%'",
                                   add_class = "panel panel-default padded-panel")
            }
          } else {
            ...
            if (!abridged) {
              ...
              chunk_idx <<- chunk_idx + 1
              vthemes::subchunkify(power_plt_ls[[sim_name]][[param_val1]][[param_val2]], i = chunk_idx)
            }
          }
          chunk_idx <<- chunk_idx + 1
        }
...
view_results(results_ls, metrics_plt_ls, metrics_errbar_plt_ls, tpr_plt_ls,
             power_plt_ls, fi_bar_plt_ls, fi_box_plt_ls, heatmap_plt_ls,
