...
ESTIMATORS_CLASSIFICATION = (
    ...
    [Model('SAPS (Include Linear)', SaplingSumClassifier, 'include_linear', True, 'max_rules', n)
     for n in cat(([30], np.arange(1, 19, 3), [25, 30]))],
    [Model('SAPS (Reweighted)', SaplingSumClassifier, 'posthoc_ridge', True, 'max_rules', n)
     for n in cat((np.arange(1, 19, 3), [25, 30]))],
...
ESTIMATORS_REGRESSION = (
    [Model('SAPS (Include Linear)', SaplingSumRegressor, 'include_linear', True, 'max_rules', n)
     for n in cat(([30], np.arange(1, 19, 3), [25, 30]))],
    [Model('SAPS (Reweighted)', SaplingSumRegressor, 'posthoc_ridge', True, 'max_rules', n)
     for n in cat((np.arange(1, 19, 3), [25, 30]))],
